...
const Card = ({ product }) => {
    ...
    const { setCarrito, carrito, addCart, active } = useContext(CartContext)
    ...
    return (
        ...
        <div className=" bg-slate-50 p-1 rounded-3xl shadow-2xl">
            <div className="relative   overflow-hidden rounded-xl p-5">
                <Link to={`product/${id}`}>
                    <img src={image} alt="Hotel Photo" className='object-cover h-48 w-full ' />

                </Link>

            </div>

            <div className="mt-1 pb-2 p-1 max-h-24 ">
                <div className='h-15'>
